...
def _krx_list(): f = _shared.get('krx_list', None); return f() if callable(f) else []
...
def search():
    ...
    _load_csvs()
    results = [
        {'name': r['name'], 'symbol': r['symbol'], 'market': r.get('market', '')}
        for r in _krx_list()
        if q in r.get('name', '').lower() or q in r.get('symbol', '').lower()
    ]
