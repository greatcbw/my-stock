            ...
            try:
                with open(path, encoding=enc) as f:
                    for row in csv.DictReader(f):
                        name = (row.get('회사명') or row.get('종목명') or
                                row.get('Company') or row.get('Name') or '').strip()
                        sym  = (row.get('종목코드') or row.get('Symbol') or
                                row.get('티커') or row.get('단축코드') or '').strip()
                        if name and sym:
                            new_list.append({'name': name, 'symbol': sym, 'market': mkt})
                break
        ...
        def generate():
            with client.messages.stream(
                ...
                for text in stream.text_stream:
                    yield text # 텍스트가 생성되는 즉시 프론트엔드로 전송

        # text/plain 타입으로 스트리밍 응답 반환
